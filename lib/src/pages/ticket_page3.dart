...
class TicketPage3 extends StatefulWidget {
  TicketPage3({Key key}) : super(key: key);

  @override
  _TicketPage3State createState() => _TicketPage3State();
}
...
class _TicketPage3State extends State<TicketPage3> {
    ...
    return StreamBuilder<MovieResponse>(
      ...
      builder: (context, AsyncSnapshot<MovieResponse> snapshot){
        if(snapshot.hasData){
          ...
          return _buildTicketPage3(snapshot.data);
  ...
  Widget _buildTicketPage3(MovieResponse data){
    List<Movie> movies = data.movies;
    return Scaffold(
      extendBodyBehindAppBar: true,
      backgroundColor: Style.Colors.mainColor,
      appBar: AppBar(
        title: Text(movies[9].title,
        style: TextStyle(
                  shadows: <Shadow>[
                    Shadow(
